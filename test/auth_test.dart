...
void main() {
  group('Mock Authentication', () {
    ...
    test('Create user should delegate to logIn funcction', () async {
      final badEmail =
          provider.createUser(email: 'tanloc@gmail.com', passwor: "123456");
      expect(badEmail, throwsA(const TypeMatcher<UserNotFoundAuthException>()));

      final badPasswordUser =
          provider.createUser(email: 'tanloc123@gmail.com', passwor: "123");
      expect(badPasswordUser,
          throwsA(const TypeMatcher<WrongPasswordAuthException>()));

      final user = await provider.createUser(email: 'foo', passwor: 'bar');
...
class MockAuthProvider implements AuthProvider {
  ...
  Future<AuthUser> createUser(
      {required String email, required String passwor}) async {
    if (!isInitialized) throw NoteInitializedException();
    await Future.delayed(const Duration(seconds: 1));
    return logIn(email: email, password: passwor);
  }
  ...
  Future<AuthUser> logIn({required String email, required String password}) {
    if (!isInitialized) throw NoteInitializedException();
    if (email == 'tanloc@gmail.com') throw UserNotFoundAuthException();
